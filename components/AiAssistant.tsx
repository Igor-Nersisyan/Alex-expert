...
Проект реализован с использованием современных LLM.
...
`;
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const apiKey = process.env.OPENROUTER_API_KEY_UII;
      
      if (!apiKey) {
        throw new Error("API Key is missing. Please make sure OPENROUTER_API_KEY_UII is set.");
      }

      // 3. Construct API Request Payload for OpenRouter (using OpenAI model)
      const payload = {
